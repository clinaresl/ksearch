...
namespace khs {
    ...
    private:
        ...
        // pointed to.
        std::vector<backpointer_t<T>> _backpointers;
    ...
    public:
        ...
        // getters
        const std::vector<backpointer_t<T>>& get_backpointers () const
            { return _backpointers; }
        const backpointer_t<T>& get_backpointer (size_t index) const
            { return _backpointers[index]; }

        // setters
        void set_backpointer (const backpointer_t<T>& backpointer)
            { _backpointers = std::vector<backpointer_t<T>>{backpointer}; }

        // operators

        // Adding inserts a new backpointer to the list of backpointers of this
        // node
        backnode_t<T>& operator+= (const backpointer_t<T>& backpointer) {
